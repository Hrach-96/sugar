...
class Home extends CI_Controller {
	...
	public function index() {
		...
		//print_r($this->session->userdata("home_search_parameters"));die;
		if(isset($_POST['btn-search'])) {
			// BASIC SEARCH MODULE
			// first clear previous serach parameters
			$this->session->unset_userdata("home_search_parameters");
	...
	public function getHomeUsers() {
		...
		if($homeUsers) {
			...
			foreach ($homeUsers as $key => $member_row) {
				...
				// is new user
				if(strtotime($member_row["user_register_date"]) >= strtotime('-1 month', strtotime($todayGMT))) {
