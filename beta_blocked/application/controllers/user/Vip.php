...
class Vip extends CI_Controller {
    ...
    public function index() {        
		$data = array();
		$this->load->model(['member_feature_model', 'vip_package_model']);
    ...
    public function buyVIP($package_id) {
        $data = array();
        $this->load->model(['payment_gateway_model', 'vip_package_model', 'oppwa_payment_token_model', 'user_model', 'country_model']);
        ...
        $checkout = json_decode($this->oppwa->prepareCheckout($package_info['package_total_amount']));
    ...
    public function oppwaResponse() {
        ...
        $this->load->model(['payment_gateway_model', 'oppwa_payment_token_model', 'vip_package_model', 'user_model', 'email_model', 'payment_transaction_model']);
        ...
        if(isset($payment_status->result->code) && $payment_status->result->code == $payment_success_code) {
            ...
            $transaction_data = array(
                ...
                'txn_id' => $checkout_id,
                'payment_gross' => $package_info['package_total_amount'],
                'currency' => $credentials['currency'],
            ...
            $this->user_model->update_user($token_info['user_id'], array('user_is_vip' => 'yes'));
            ...
            if($this->db->trans_status() === FALSE) {
                ...
                $data['purchase_amount'] = $package_info['package_total_amount'];
