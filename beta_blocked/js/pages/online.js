...
$(document).ready(function() {
	...
	if(userLoggedIn == true) {
		...
		setTimeout(loadOnlineUsers(currOnlineUsersPage), 2000);	
	}
				...
				if($('#onlineUsersList').html() != undefined) {
					...
					if(response.total_online_users > 0) {
						for(ind=0; ind < response.total_online_users; ind++) {
							...
							var imgObj = $('#img_' + response.online_users_hash[ind].user_id).html();
							if($('#is_home_page').html() != undefined) {
								if(imgObj == undefined) {

			                        var home_users = '<div class="col-xs-18 col-sm-6 col-md-3 col-xs-6 isOnineNow">\n\
			                            <div class="profile_thumbnail profile_data" rel="'+ response.online_users_hash[ind].user_id_encrypted +'">\n\
			                                <a id="img_'+ response.online_users_hash[ind].user_id +'" href="' + base_url + 'user/profile/view?query=' + response.online_users_hash[ind].user_hash + '" class="profile_a">\n\
			                                ...
			                                <div class="caption">\n\
			                                    <div class="row">\n\
			                                        <div class="col-md-12 col-sm-12 col-xs-12">\n\
			                                            <h4 class="pro_name">\n\
			                                                ' + response.online_users_hash[ind].user_access_name + ' | \n\
			                                                ' + response.online_users_hash[ind].user_age + '\n\
			                                            </h4>\n\
			                                        </div>\n\
			                                        <div class="col-md-12 col-sm-12 col-xs-12">\n\
			                                            <h4 class="loc_km">\n\
			                                                <span class="km">\n\
			                                                    ' + response.online_users_hash[ind].distance + 'km - \n\
			                                                    <span class="city">\n\
			                                                        ' + response.online_users_hash[ind].user_city + '\n\
			                                                    </span>\n\
			                                                </span>\n\
			                                            </h4>\n\
			                                        </div>\n\
			                                        ...
			                                        <div class="actions_div">\n\
			                                            <ul class="profile_actions">\n\
			                                                <li>\n\
			                                                    ...
			                                                    if(response.online_users_hash[ind].is_kissed == true) {
			                                                        home_users += 'active-icon-red';
			                                                    }
			                                                    home_users += '"></i>\n\
			                                                    <i class="flaticon-like ';
			                                                    if(response.online_users_hash[ind].is_favorite == true) {
		                            ...
		                            if(response.online_users_hash[ind].user_is_vip == 'yes') {
	                        		...
	                        		if($('#img_' + response.online_users_hash[ind].user_id).find('span').hasClass('onoffStatusSpn') == false) {
	                        			$('#img_' + response.online_users_hash[ind].user_id).prepend('<span class="onoffStatusSpn os">' + ln_online_str + '</span>');

	                        			cobj = $('#img_' + response.online_users_hash[ind].user_id).closest('.col-xs-18');
				...
				if(response.errorCode == 0) {
					for(i=0; i < response.data.length; i++) {
						...
						if(userAdded == null) {
						    ...
						    if(response.data[i].last_online_time < '00:03:00') {
