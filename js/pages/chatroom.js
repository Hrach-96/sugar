					...
					if(response.status == true) {
						...
						if(response.errorCode == 0) {
							for(i=0; i < response.data.length; i++) {
								...
								user_chat_text += '<div class="msgDiv"><p class="wordWrapBreakWordClass">'+ response.data[i].message_text +'</p>\n\
									</div>\n\
								</li>';
							}
						}
		...
		$.ajax({
			...
			success: function(response)  {
				...
				if(response.status == true) {
					...
					user_chat_text += '<div class="msgDiv"><p class="wordWrapBreakWordClass">'+ response.messageText +'</p>\n\
						</div>\n\
					</li>';
				...
				if(response.status == true) {
					for(i=0; i < response.data.length; i++) {
						user_chat_text += '<li class="sent">\n\
							...
							<div class="msgDiv"><p class="wordWrapBreakWordClass">'+ response.data[i].message_text +'</p>\n\
							</div>\n\
						</li>';
					}						
