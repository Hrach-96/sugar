...
class News_model extends CI_Model
{
	...
	public function remove_individual_email($id)  {
        $this->db->delete($this->table_news_subscribers, array('news_subscriber_id' => $id));
        return true;
	}	
	...
	public function get_news_subscribed_info_by_id($id) {
		$this->db->from($this->table_news_subscribers);
		$this->db->where('news_subscriber_id', $id);
		$Q = $this->db->get();
	...
	public function get_all_subscribers_list_new($per_page, $offset,$search_email) {
		$this->db->from($this->table_news_subscribers);
		if($search_email != '') {
			$this->db->like('news_subscriber_email', $search_email);
		}
		$this->db->order_by('news_subscriber_id', 'desc');
		$this->db->limit($per_page, $offset);
		$Q = $this->db->get();
	...
	public function count_all_subscribers_list_new($search_email) {
		$this->db->from($this->table_news_subscribers);
		if($search_email != '') {
			$this->db->like('news_subscriber_email', $search_email, 'both');
		}
		$Q = $this->db->get();
