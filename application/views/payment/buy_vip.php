...
<?php
	...
	if($currency != 'Euro'){
		$getPackageMultCurrency = $this->vip_package_model->get_package_multi_currency($currency_variable,$package['package_id']);
	}
	if(isset($getPackageMultCurrency)){
		$total_price = number_format((float)$getPackageMultCurrency->value, 2, '.', '');
	}
	else{
		$total_price = $package['package_total_amount'];
	}
?>
<!--Buy VIP Modal-->
						...
						<div class="row email_box col-sm-12" id="user-personal-infomation">
                            ...
                            <div class="row">
                                <div class="col-md-12 text-center" style="padding-bottom: 30px;">
									<button class="buy_nw_credit btn_hover btn-save-per-details" type="button"><?php echo $this->lang->line('continue');?></button>
								</div>
							</div>
						</div>						
...
<!-- 									<div class="lilo-accordion-control">
									...
									<div class="lilo-accordion-content">
										<form role="form" action="<?php echo base_url('stripe/stripePostBuyVip'); ?>" method="post" class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="<?php echo $this->config->item('stripe_key') ?>" id="payment-form">
					                        <input type='hidden' name='amount' value="<?php echo $total_price; ?>" >
					                        <input type='hidden' name='description' value="<?php echo $package['package_validity_total_months']; ?>" >
