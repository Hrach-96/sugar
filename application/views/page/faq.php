			...
			<div class="col-md-12 padding_zero">
				<div class="your-class">
					...
					<div class="lilo-accordion-content">
						...
						<ul class="fqUl padding_zero sugarbabe_zero_male">
						<?php 
						...
						<ul class="fqUl padding_zero sugarbabe_zero_female">
						<?php 
					...
					<div class="lilo-accordion-content">
						...
						<table class="creditsTbl credit_table_prices_faq">
							<?php 
				...
				<form class="contact_form" action="<?php echo base_url('page/faq'); ?>" method="POST">	
					<div class="text-left email_box">
						<div class="row">
							...
							<div class="col-md-6">
								<img id="captcha_image_url" alt='Captcha' src="<?php echo $captcha_image_url; ?>" style="margin-top: 10px;width:100%;height:45px;" /></label>
							</div>
							<div class="col-md-6">
...
<script type="text/javascript">
	$(document).ready(function(){
		var select_lang = $(".select_lang ").html();
		if(select_lang == 'CH' || select_lang == 'UK'){
			var currency = '';
			if(select_lang == 'CH'){
				currency = 'CHF';
			}
			...
			$.ajax({
				url: base_url + "page/get_currency",
				type: 'POST',
				data: {select_lang: select_lang},
				success: function(resp) {
					resp = JSON.parse(resp)
					if(resp.length > 0){
						$(".sugarbabe_zero_female").html('');
						var female_html = '';
						var male_html = '';
						for(var i = 0 ; i < resp.length ; i++){
							if(resp[i]['gender'] == 'female'){
								if(resp[i].package_validity_total_months == 1){
									female_html += '<li class="fourteen os">' + resp[i].package_validity_total_months + " <?php echo $this->lang->line('month'); ?> = " + resp[i].value + " " + currency + "</li>";
								}
								else{
									female_html += '<li class="fourteen os">' + resp[i].package_validity_total_months + " <?php echo $this->lang->line('month'); ?> = " + parseFloat((resp[i].value/resp[i].package_validity_total_months)).toFixed(2) + " "  + currency + " <?php echo $this->lang->line('per_month'); ?>" + " / <?php echo $this->lang->line('total'); ?> " +   + parseFloat((resp[i].value)).toFixed(2) + " " +currency + "</li>";
								}
							}
							else if(resp[i]['gender'] == 'male'){
								if(resp[i].package_validity_total_months == 1){
									male_html += '<li class="fourteen os">' + resp[i].package_validity_total_months + " <?php echo $this->lang->line('month'); ?> = " + resp[i].value + " " + currency + "</li>";
								}
								else{
									male_html += '<li class="fourteen os">' + resp[i].package_validity_total_months + " <?php echo $this->lang->line('month'); ?> = " + parseFloat((resp[i].value/resp[i].package_validity_total_months)).toFixed(2) + " "  + currency + " <?php echo $this->lang->line('per_month'); ?>" + " / <?php echo $this->lang->line('total'); ?> " +   + parseFloat((resp[i].value)).toFixed(2) + " " +currency + "</li>";
								}
							}
						}
						$(".sugarbabe_zero_female").html(female_html)
						$(".sugarbabe_zero_male").html(male_html)
					}
				}
			});
			$.ajax({
				url: base_url + "page/get_currency_for_credit",
				type: 'POST',
				data: {select_lang: select_lang},
				success: function(resp) {
					resp = JSON.parse(resp);
					if(resp.length > 0){
						$(".credit_table_prices_faq").html('');
						var html_credit_multicurrency = '';
						for(var i = 0 ; i < resp.length ; i++){
							html_credit_multicurrency += "<tr>";
								html_credit_multicurrency += "<td>" + resp[i].package_credits + " Credits</td>";
								html_credit_multicurrency += "<td>" + currency + " " + resp[i].value + "</td>";
							html_credit_multicurrency += "</tr>";
						}
						$(".credit_table_prices_faq").html(html_credit_multicurrency);
					}
				}
			});
		}
