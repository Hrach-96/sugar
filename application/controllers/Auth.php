...
class Auth extends CI_Controller {
			...
			$data["error_array"] = $this->form_validation->error_array();
	         ...
	         if ($this->input->post('captcha_text') != $this->session->userdata('captcha_text')) {
	        	...
	        	if($user_id) {
					...
					$user_city = $this->input->post('locality');
		        	...
		        	if($this->db->trans_status() === FALSE) {
                        ...
                        $data['receiver_user_name'] = $this->input->post('username');
    ...
    function testVErifiyEmail(){
    	$this->load->model(['user_model','language_model']);
        $this->lang->load('user_lang', $this->session->userdata('site_language'));
    	$this->lang->load('site_lang', 'german');
		$verify_url = base_url('auth/verify?hash='.urlencode('aaa.dddd@gmail.com'));		        		
        $subject = $this->lang->line('account_created_verify_your_email');
        $data['user_pic'] = base_url("images/avatar/male.png");
        $data['user_email_verify_url'] = $verify_url;
        $data['email_template'] = 'email/user_registration';
        $data['receiver_user_name'] = 'dddd';
        $email_message = $this->load->view('templates/email/main', $data, true);
        // new Version
        $this->load->library('email');
		$this->email->from('support@sugarbabe-deluxe.eu', 'Sugarbabe-Deluxe');
		$this->email->set_mailtype("html");
		$this->email->to('jigyar.jigyarov@gmail.com');
		$this->email->subject($subject);
		$this->email->message($email_message);
		$this->email->send();
    }
	...
	public function fb_redirect_login() {
        if (isset($_GET['code'])) {
        	$this->load->model(['user_model','auth_model']);
        	$settings = $this->session->userdata('site_setting');
            $params = array(
                'client_id'     => $this->config->item('fb_client_id'),
                'redirect_uri'  => $this->config->item('fb_redirect_uri'),
                'client_secret' => $this->config->item('fb_client_secret'),
                'code'          => $_GET['code'],
            );
            ...
            if (isset($tokenInfo['access_token'])) {
                ...
                $userInfo = json_decode(file_get_contents('https://graph.facebook.com/me?fields=id,email,name,last_name,first_name,picture&access_token='.$tokenInfo['access_token']), true);  
     
                // $userInfos = json_decode(file_get_contents('https://graph.facebook.com/v7.0/me/taggable_friends?access_token='.$tokenInfo['access_token']), true);
                if(isset($userInfo['email']) && $userInfo['email'] != ''){
                	print "<pre>";
                	var_dump($userInfo);die;
	                if (isset($userInfo['id'])) {
	                	$userExist = $this->user_model->get_user_info_using_email_or_username($userInfo['email']);
	                	if($userExist){
	                		print "<pre>";
		                    ...
		                    $login_register_array = array(
				        		'user_access_name' => $userInfo['name'],
				        		'user_password' => '',
	        				...
	        				if($user_id){
								...
								$user_information_array = array(
					        		...
					        		'user_city' => 'Munich',
					        		'user_latitude' => '48.137154',
					        		'user_longitude' => '11.576124',
					        		'language_id_ref' => '1',
					        		'user_active_photo' => NULL,
					        		'user_active_photo_thumb' => NULL,
