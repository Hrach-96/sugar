...
class Approvals extends CI_Controller {
	...
	public function approve_user_content() {
		$this->load->model(['user_content_model', 'user_model', 'email_model', 'photo_model']);
		...
		if($req_info) {
			if($req_info['content_status'] == 'pending') {
            	...
            	if($this->db->trans_status() === FALSE) {
		            ...
		            $profile_pic = $this->photo_model->get_active_user_profile_pic($req_info['content_user_id']);
		            if($profile_pic) {
		...
		if($req_info) {
			if($req_info['photo_status'] == 'inactive') {
            	...
            	if($this->db->trans_status() === FALSE) {
	                ...
	                $data['user_profile_pic'] = base_url("images/avatar/".$member_info['user_gender'].".png");
		            $profile_pic = $this->photo_model->get_active_user_profile_pic($req_info['photo_user_id_ref']);
		            if($profile_pic) {
		                $data['user_profile_pic'] = base_url($profile_pic['photo_thumb_url']);
		            }
