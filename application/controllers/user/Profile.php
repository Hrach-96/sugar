...
class Profile extends CI_Controller {
    ...
    public function view() {        
        ...
        $this->load->model(['user_model', 'visitor_model','email_model','photo_model','kiss_model', 'favorite_model','dislike_model','report_user_model','question_model', 'unlock_request_model', 'sport_model','language_model', 'interest_model','contact_request_model']);
        ...
        if($user_id != $member_id) {
            $checkAlreadyVisited = $this->visitor_model->get_user_profile_visited_info($user_id, $member_id);
                $visitor_info = $this->user_model->get_active_user_information($user_id);
                // Send Email if profile viewed
                $data['user_name'] = $data["user_row"]['user_access_name'];
                $data['user_profile_pic'] = base_url("images/avatar/".$visitor_info['user_gender'].".png");
                ...
                if($data["user_row"]['user_country'] == 'United Kingdom'){
                    $this->lang->load('user_lang', 'english');
                }
                else{
                    $this->lang->load('user_lang', 'german');
                }
                $subject = $this->lang->line('hello') . " " . $data["user_row"]['user_access_name'] . "! " . $this->lang->line('you_are_very_popular');
                $data['email_template'] = 'email/visit_profile';
                ...
                $this->email->to($data["user_row"]['user_email']);
    ...
    public function cancelVIPMembership() {
        $this->load->model(['user_model', 'Vip_package_model', 'email_model']);
        $this->lang->load('user_lang', $this->session->userdata('site_language'));
        ...
        if(isset($_POST['cancel_vip_token'])) {
            ...
            if($success) {
                $userInfo = $this->user_model->get_user_information($user_id);
                $update_array_canceled_date = [
                    'user_id' => $user_id,
                    'canceled_date' => date('Y-m-d'),
                    'buy_vip_id_col' => $userInfo['buy_vip_id_num'],
                ];
                $this->user_model->insert_user_canceled_vip_date($update_array_canceled_date);
