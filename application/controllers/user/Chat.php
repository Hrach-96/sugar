...
class Chat extends CI_Controller {
    ...
    public function sendChatMessage() {
        ...
        if($msg_text != '') {
            ...
            $message_array_info = array(
                'message_sender_id' => $user_id,
                'message_receiver_id' => $member_id,
                'last_message_sent_date' => date('Y-m-d H-i-s'),
            );
            $getUserChatInfo = $this->chat_model->get_user_chats_info($user_id,$member_id);
            if($getUserChatInfo){
                $day1 = $getUserChatInfo->last_message_sent_date;
                $day1 = strtotime($day1);
                ...
                if($diffHours > 5){
                    ...
                    $this->chat_model->update_chat_user_info($user_id,$member_id,$data);
                    $this->sendEmailEachTimeNotification($user_id,$member_id);
                }
            }
            else{
                $this->chat_model->add_chat_user_info($message_array_info);
                $this->sendEmailEachTimeNotification($user_id,$member_id);
            }
    ...
    public function sendEmailEachTimeNotification($message_sender_id,$message_receiver_id) {
        $this->load->model([ 'email_model','user_model']);
        $senderInfo = $this->user_model->get_user_information($message_sender_id);
        $receiverInfo = $this->user_model->get_user_information($message_receiver_id);
        $this->lang->load('user_lang', $this->session->userdata('site_language'));
        $to_email = $receiverInfo['user_email'];
        $subject = $this->lang->line('you_have_received_a_chatmessage_from') . " " . $senderInfo['user_access_name'] . " " . $this->lang->line('erhalten_only_deutch');
        $data['user_profile_pic'] = base_url("images/avatar/".$senderInfo['user_gender'].".png");
        if($senderInfo['user_active_photo_thumb'] != '') {
            $data['user_profile_pic'] = base_url($senderInfo['user_active_photo_thumb']);
        }
        $data['sender_name'] = $senderInfo['user_access_name'];
        $data['receiver_name'] = $receiverInfo['user_access_name'];
        $data['email_template'] = 'email/you_have_unread_message';
