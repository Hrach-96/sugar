...
class Credits extends CI_Controller {
    ...
    public function index() {
        ...
        $data['user_info'] = $this->user_model->get_user_information($user_id);
    ...
    public function buyCreditVoucher($package_id) {
        $settings = $this->session->userdata('site_setting');
        $settings = $this->session->userdata('site_setting');
        $data["settings"] = $settings;
        $data["title"] = $settings["site_name"] . " - " . $settings["site_tagline"];
        ...
        $this->lang->load('user_lang', $this->session->userdata('site_language'));
        $this->load->view('payment/buy_credit_voucher', $data);
    }
    public function buyCredit($package_id) {
        $this->load->helper('cookie');
        $data = array();
        $this->load->model(['payment_gateway_model', 'credit_package_model', 'oppwa_payment_token_model', 'user_model','voucher_model']);
        $voucher_code = '';
        ...
        if($voucher_code != ''){
            ...
            if(!is_null($voucher_info)){
                $this->session->set_userdata('used_voucher_credit', $voucher_info->id);
                $package_amount = $package_amount - (($voucher_info->procent / 100) * $package_amount);
                $package_amount = number_format($package_amount, 2, '.', '');
            }
        }
        $checkout = json_decode($this->oppwa->prepareCheckout($package_amount));
        ...
        $package_info['package_amount'] = $package_amount;
        $data['package'] = $package_info;
        $this->load->view('payment/buy_credit', $data);
    }

    public function oppwaResponse() {
        $this->load->model(['payment_gateway_model', 'credit_package_model', 'oppwa_payment_token_model', 'user_model', 'email_model', 'payment_transaction_model','voucher_model']);
